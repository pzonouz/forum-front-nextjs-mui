...
import { FileSearchListView } from "./FileSearchListView";

const FileSearchComponent = async ({ query }: { query: string }) => {
  return (
    <Box
      sx={{
        width: "80%",
    ...
    >
      ...
      <Suspense fallback={<LoadingQuestion />}>
        <FileSearchListView query={query} />
      </Suspense>
    </Box>
  );
};
export { FileSearchComponent };
